...
use crate::simulation::animals::{Wolf, Deer, AnimalAI};
use crate::simulation::combat::{Health, Body};
    ...
    egui::show_tooltip_at_pointer(ctx, egui::LayerId::debug(), tooltip_id, |ui: &mut egui::Ui| {
        ...
        if !entities.is_empty() {
            ...
            for &entity in entities {
                if let Ok((ai, needs, mood, sex, faction, agent, body)) = person_query.get(entity) {
                    let name = name_query.get(entity).map(|n| n.as_str()).unwrap_or("Person");
                    ...
                    let kind = if wolf.is_some() { "Wolf" } else if deer.is_some() { "Deer" } else { "Animal" };
