...
**Bad-step fallthrough.** When the fast path's emitted path fails `first_invalid_step` against live `ChunkMap` (cached `cell_z` drifted past a terrain mutation), the worker sets `hotspot_fastpath_bad_step` and **falls through to A***, rather than returning `NoRouteStepContinuity`. Stale flow-field cells are a recoverable cache miss; A* re-routing against live state is the authoritative fallback. A* itself emitting a bad-step path still fails the request — that's a real planner/runtime inconsistency. Counter `PathfindingDiagnostics::hotspot_fastpath_bad_steps` (distinct from `hotspot_fastpath_misses`) measures invalidation drift in-game; see `plans/fix-sleep-stalls.md` Deferred follow-up for the underlying invalidation gap.
