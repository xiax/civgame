...
- **`ThirstScorer`** (Survival) fires `AgentGoal::Drink`; `htn_drink_dispatch_system` tries inventory `clean_water` first, then nearest **water-bearing** `WellMap` entry (chebyshev-adjacent stand tile, treated as clean by default; `nearest_well_tile` skips dry wells via `drink::well_has_water` so agents fall through to the river method rather than fixate), then `nearest_fresh_drinkable_tile` (chebyshev rings; skips Salt **and Brackish** via `world::biome::water_kind_at(...).is_drinkable()`; severe widens scan 2×). **Phase 6 salinity:** `WaterKind::{Fresh,Brackish,Salt}` from `Globe::salinity_at` (Endorheic basins brackish 0.6 → non-drinkable; ocean salt; rivers/open lakes fresh); `is_drinkable()` = Fresh-only is the single rejection rule (drink + animal water-seek). **Aquifer wells:** `well_has_water`/`well_reaches` — a well yields only when `HydroCell.aquifer_level` is within the `WELL_REACH_Z=4` shaft below `surface_z`; over a deep/arid table the well is dry → graceful `DrinkOutcome::WellDry` (no quench, agent re-plans), and an all-dry-well village already reads "fresh water far" so existing `WaterAccess` pressure responds (no new event bus).
...
- **`carve_plots_system`** (FixedUpdate Economy, after `settlement_planner_system`, before `chief_directive_system`): re-carves on `culture_hash` mismatch, values via `compute_plot_value`. Plot sizes: Residential 6×6, Crafting/Storage 4×4, Agricultural **16×16** (matches organic parcel size 1:1 — see Farming below); Civic/Sacred/Market/Defense kept whole. For Agricultural plots it also records every tile in `PlotIndex.ag_tiles` and flips Grass/soil tiles to `TileKind::Cropland` (`ResMut<ChunkMap>` + `TileChangedEvent`); idempotent across re-carves, stale teardown drops `ag_tiles` (the Cropland tile intentionally stays — abandoned fields look tilled).
...
- **Plot-scoped chief Farm postings:** `chief_job_posting_system` posts one `JobKind::Farm` per `FarmPlotAssignments` entry with `JobProgress::Planting { plot_id: Some, assigned_farmer: Some, area: plot.rect, … }` whenever plantable seeds are in storage; low grain raises priority through `farm_pressure` but no longer gates posting. **Open-plot fallback (NOT `home_tile ±5`):** the daily assignment refresh needs a `Profession::Farmer` to already exist, so when no assigned posting fires the chief posts an OPEN plot-scoped job for the nearest unassigned StateOwned Agricultural plot (`plot_id: Some`, `assigned_farmer: None` → any farmer claims it, `resolve_farm_scope` → Communal → planting restricted to the plot rect = belt). **No Agricultural plot ⇒ NO Farm posting** — farming waits for the belt to carve rather than tilling the town centre (the old `plot_id=None` / `home_tile±5` bootstrap that put farmers in the middle of the settlement is deleted).
