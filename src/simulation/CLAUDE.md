...
- **Plot-restricted planting search:** `htn_plant_from_storage_dispatch_system` reads the worker's `JobClaim` posting; if `plot_id.is_some()`, restricts planting tile search to `find_nearest_unplanted_in_rect(plot.rect, …)`. Falls through to the radius-15 search for bootstrap and non-claim cases. (TODO: per-household storage source override + harvest plot-filter for `MemoryKind::AnyEdible` are deferred to a follow-up.)
