...
**Dams mirror this pipeline exactly** (Phase 4 of `plans/water-simulation.md`). `BuildSiteKind::Dam` → `TileKind::Dam=25` over a `River`/`Water` cell + `Dam { faction_id, tile, restore_tile, crest_z }` in `DamMap`. The **entity is the durable truth**; the tile kind is its cache projection, restamped from `DamMap` on chunk reload by `world::water_runtime::restamp_runtime_water_on_chunk_load` (shared `stamp` closure with Bridge). Recipe `6 stone + 4 wood`, 180 work, `BRIDGE_BUILDING`-gated (dedicated `DAM_BUILDING` = v2; `BuildRecipeIdx::Dam` appended last so existing vec positions stay stable). Unlike Bridge, the cell is **not** water-like/fresh/drinkable — water is blocked. Finalize calls `RuntimeWater::register_dam(tile, crest_z=bp.target_z)` (recorded in `RuntimeWater.dam_crests` — a cache of `DamMap`, consumed by the Phase 5 fluid sim to pool water upstream). Deconstruct shares the generalised `water_anchored_refund_tile` path with Bridge: restores the prior tile, `RuntimeWater::clear_dam`, refunds `3 stone + 2 wood` on the nearest bank. `FurnitureMaps` bundles `dam_map` + `runtime_water` (under the 16-param cap). **v1 is player-built only** (right-click Build Dam on River/Water) — no AI dam emitter (deferred v2).
