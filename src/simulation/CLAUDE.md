...
- **Plot-scoped chief Farm postings:** `chief_job_posting_system` posts one `JobKind::Farm` per assignment with `JobProgress::Planting { plot_id, assigned_farmer, area: plot.rect, … }` whenever plantable seeds are in storage; low grain raises priority through `farm_pressure` but no longer gates posting. Bootstrap fallback (`plot_id = None`, `home_tile ±5`) only fires when no plot-scoped posting can be emitted.
