...
- **`ThirstScorer`** (Survival) fires `AgentGoal::Drink`; `htn_drink_dispatch_system` tries inventory `clean_water` first, then `nearest_fresh_drinkable_tile` (chebyshev rings, skips salt via `world::biome::water_kind_at`; severe widens scan 2×).
- **`drink_task_system`** consumes from hands or sips an adjacent fresh tile; raw drinks bump `Sickness.severity` 60, contaminated tiles 140.
...
- **External preempts** (`dispatch_player_command_system`, hunter demote, stale reset) call `aq.cancel()`, dropping both `current` and queue.
...
- **Composite footprints (`BuildIntent::CompositeHouse`)**: `plan_composite_building` walks `building_template::shape_tiles(shape, anchor, rotation)`, classifies each cell as Wall / Door / Bed. Chalcolithic+ rolls 10% `LShape` farmstead when `bed_deficit ∈ [2,4)`. UShape exists but isn't auto-emitted.
...
`OnEnter(Playing)` chain: `spawn_population → auto_found_default_settlements_system → kickoff_initial_survey_system → seed_starting_buildings_system → clear_obstacles_under_seeded_structures → mark_warmup_complete_system`.

- Paleo/Meso: deterministic band-camp seeder (`paleolithic_hearth_positions_river_aware` + `seed_paleo_beds_around_hearth`).
- Neolithic+: unified intent loop — drives runtime `generate_candidates` and applies via `seed_apply_intent` (direct stamp, no Blueprint+worker round-trip). Same intent stream as `chief_directive_system`; only the resolver differs. Civic structures gate at `peak_pop = member_count` (no grandfather behaviour).
...
- **`carve_plots_system`** (FixedUpdate Economy, after `settlement_planner_system`, before `chief_directive_system`): re-carves on `culture_hash` mismatch, values via `compute_plot_value`. Plot sizes: Residential 6×6, Crafting/Storage 4×4, Agricultural 10×10; Civic/Sacred/Market/Defense kept whole.
...
- **`land_listing_system`** (Economy, every `TICKS_PER_DAY/4`): publishes Sale + Lease listings for `StateOwned` plots when `LandPolicy` permits. Civic-class zones skipped. Cap `TARGET_LISTINGS_PER_FACTION = 8`. Sale asking = `base_value`; lease asking = `base_value × rent_yield_pct` per month, floored at `MIN_MONTHLY_RENT`.
- **`household_land_acquisition_system`** (Economy, daily): plot-less household with `treasury ≥ HOUSEHOLD_MIN_TREASURY_FOR_LEASE` browses parent listings. Affordability: lease ≤ 40%, buy ≤ 70% of treasury. Preference: Sale → Lease → Sharecrop. Also claims nearest unowned same-village Agricultural plot within 12 chebyshev as child (mirrored tenure).
...
- **`MegaChunkCoord`:** `MEGACHUNK_SIZE_CHUNKS=16`. Independent of climate cells.
