...
impl UtilityNet {
    pub fn new_random() -> Self {
        ...
        for i in 0..Q_H1 {
            for j in 0..Q_IN {
                net.w1[i][j] = (fastrand::f32() - 0.5) * 0.2;
            }
            net.b1[i] = (fastrand::f32() - 0.5) * 0.2;
        }
        for i in 0..Q_H2 {
            for j in 0..Q_H1 {
                net.w2[i][j] = (fastrand::f32() - 0.5) * 0.2;
            }
            net.b2[i] = (fastrand::f32() - 0.5) * 0.2;
        }
        for i in 0..Q_H2 {
            net.w3[i] = (fastrand::f32() - 0.5) * 0.2;
        }
    ...
    pub fn from_parent(parent: &UtilityNet) -> Self {
        ...
        for i in 0..Q_H1 {
            for j in 0..Q_IN {
                child.w1[i][j] += gauss_noise();
            }
            child.b1[i] += gauss_noise();
        }
        for i in 0..Q_H2 {
            for j in 0..Q_H1 {
                child.w2[i][j] += gauss_noise();
            }
            child.b2[i] += gauss_noise();
        }
        for i in 0..Q_H2 {
            child.w3[i] += gauss_noise();
        }
    ...
    pub fn evaluate_plan(&self, state: [f32; STATE_DIM], plan_feat: [f32; PLAN_FEAT_DIM]) -> f32 {
        let mut input = [0.0f32; Q_IN];
        input[..STATE_DIM].copy_from_slice(&state);
        input[STATE_DIM..].copy_from_slice(&plan_feat);
