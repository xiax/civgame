...
- **`SettlementBrain`** — one per full settlement. Tracks `phase`, anchors (hearth, water, storehouse, field, shrine, workshop, market, gate, high ground, material patch), soft districts, traffic heat, planned `road_segments`/`road_tiles`, frontier tiles, organic parcels, deterministic seed, and survey ticks. Runtime surveys are async declarative diffs via `survey_task.rs`: snapshot bounded terrain/structure/member/faction inputs, compute on `AsyncComputeTaskPool`, then validate and apply a small number of ready brains/road pushes per tick. Pressure/morphology/project-selection systems remain main-thread `SimulationSet::Economy` validation.
...
- **Doormat reservations** (`simulation/doormat.rs`): every door registers its 1-tile cardinal-outside neighbour in `DoormatReservations`. `is_clear_footprint` / `plot_rect_vacant` / `find_palisade_site` / `find_clear_tile_in_zone` / `find_unfilled_civic_zone_tile` / `find_bed_tile_around_hearth` / `seed_farmstead_yard` all refuse reserved tiles, so no neighbour wall / palisade / yard can sit on a door's opening side. Door spawn writes the doormat tile to `TileKind::Road` directly (`write_road_tile`) and pushes a `RoadCarveJob::Connector { doormat, home }` onto `RoadCarveQueue`. `road_carve_system` plans the join at carve time via `find_door_connector` — a bounded **4-connected** search returning a guaranteed-carvable cardinal path to the nearest carved/planned spine tile (or a stub toward home), routing around structures + farms. The result is an unbroken, cardinal-continuous road into the spine; no diagonal-only touch, no isolated doormat. The `Door` `on_remove` hook (registered alongside `JobEscrow`'s) frees the reservation on demolition.
...
- **Farming startup + posting** (`farm.rs`, `jobs.rs`): `seed_starting_farms_system` creates a 16×16 `Agricultural` plot and places `STARTING_GRAIN_SEEDS=32` as physical `GroundItem`s on faction storage, because `FactionStorage.totals` is only a rollup cache. Chief Farm postings are gated by crop tech, grain policy, members, and available seeds — not by low stored grain — so stocked societies still plant the next cycle. **Foreman pass:** `chief_job_posting_system` ranks state-owned Ag plots via `plot_focus_score` (Started/plantable > Unprepared > Idle; ties broken on chebyshev-to-home then rect coord) and emits Spring Prepare/Plant **only for the focused plot**. Overflow plots wait until the focused plot's live FieldWork postings saturate (`claimants.len() >= posting_target_workers`); the 60-tick `CHIEF_POSTING_INTERVAL` re-runs the gate so the next plot opens within ~3 s of saturation. Autumn `Harvest` ignores the overflow gate — mature crops are deadline-sensitive — but still walks plots sorted nearest-first. Seed budget is allocated to plots in sorted order so the focused plot commits seed before any overflow plot. `chief_farm_plot_assignment_system` matches farmers to plots in stable rect-coord order with chebyshev-nearest + entity-id tie-break. Claim-side tightening: `FARM_DIST_PENALTY = 0.01` (10× the global `0.001`) so the unpaid-posting score pulls a worker to the nearest equivalent FieldWork posting without overwhelming profession bias.
...
- Fixed update: **20 Hz** (`main.rs`). One game-day is `TICKS_PER_DAY = 7200` ticks ⇒ 360 real-seconds at 1× speed. Continuous physiology (`needs.rs`, `energy.rs`, `animals.rs`, play) is calibrated **per game day** through `world::seasons::per_game_day_rate`, so daily totals stay invariant when `TICKS_PER_DAY` changes — never introduce a new local day constant; widen `TICKS_PER_DAY` and add a per-day target.
