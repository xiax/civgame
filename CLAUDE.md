...
- `ChunkMap::vertical_clearance_at(x, y)` counts open `Air`/`Ramp` Z-levels above the surface — vertical clearance for tall multi-Z vehicles (`pathfinding::vehicle_path::footprint_astar`).
...
- **`Cropland`** — tilled farm soil. **Only ever appears inside an Agricultural plot.** **Worked into existence** by `farm::prepare_field_task_system` (Sequential, `FIELD_PREP_WORK_TICKS=80`). `carve_plots_system` populates `PlotIndex.ag_tiles` + per-tile `farm::FieldTileIndex` entries but leaves the underlying soil/grass; founders pay Spring 1 to till. At game start the carve runs **inside the `OnEnter(Playing)` chain** (survey → project plans → carve → seed-farms, see `plans/spawn-farm-seeding.md`), so `seed_starting_farms_system` finds a real carved Agricultural plot to stamp its bounded contiguous starter patch into (Cropland kind only — natural world-gen fertility, never overridden). There is no seed-time house yard — personal kitchen gardens are real Agricultural parcels (see `organic_settlement::append_dwelling_gardens`). `is_soil_like`, speed 0.9, and **never paved by road carving**.
...
- **Vehicle part sprites** (`rendering/vehicle_part_sprites.rs`) — hand-drawn ASCII art for every `VehiclePartKind`, visually distinct variants, and multi-cell weapon-module composites; registered into `SpriteLibrary` from `setup_sprite_library`. Resolver `vehicle_part_sprite_key(kind, variant_label, view)` + `vehicle_module_sprite_key(module_label, view)`; `view_for_heading` maps the 0..4 heading to `(Side|Front, flip_x)` (two views: side drawn facing east, west flips horizontally; N/S share the front view). The sprite-aware `entity_sprites::vehicle_sprite_plan_with_data(design, heading, &VehicleData)` populates `VehicleSpriteCell.{sprite_key, fallback_sprite_key, flip_x}`; `spawn_vehicle_sprites` tries variant→base→colour-quad in that order. Multi-cell weapon modules collapse to one composite sprite at the anchor cell (smallest `(z, y, x)`), sized to the module footprint × 16 px. The legacy colour-only `vehicle_sprite_plan` survives as a backward-compatible wrapper for the designer's egui preview and headless tests.
- **Water-current streaks** (`rendering/water_current_render.rs`) — animated flow-streak `Sprite`s on flowing-`River` tiles: a bright dash scrolls downstream along the `world::water_current::WaterCurrentField` direction and fades at the cycle ends (`animate_current_streaks_system`). `water_current_render_system` **reconciles** the per-chunk `CurrentStreakIndex` against the field every frame (not event-driven — the spawn-area chunks pre-load without a `ChunkLoadedEvent`). Streaks sit at z=0.35 above tiles, carry `ProjectedAnchor::Static` for tilted view. First per-tile sprite overlay.
...
Functional tools: `ToolForm` (Knife/Axe/Pick/Hammer/Sickle/Awl/FishingKit) decides *what*; an `Item`'s material+quality → `ToolTier` (`Bone<Stone<FineStone<Copper<Bronze`) decides *how well* via `work_speed_mult`. Tools ride in a `ToolKit` component (`Vec<Item>`, era-scaled `capacity_for_era`) separate from `Carrier` hands; spawned on every Person. **Hard gates** (graceful — *no* `ToolKit` component reads as satisfied; an *empty* kit blocks): Axe to fell trees (no-Axe → deadwood, tree not despawned), Pick to mine Stone/Wall/Ore (no-Pick → trickle, tile not carved) + dig stone-like floors, Sickle to reap mature Grain, Fishing Kit to fish, recipe `tool_requirements` to craft. `seed_starting_tools_system` (OnEnter) pre-stows era loadouts into member kits + storage. Tool acquisition: `Task::WithdrawTool`→`StowToolKit` chain. Faction demand: `compute_faction_tool_deficits` feeds chief craft postings (generic `tools` commodity stays only as the Ard-Plow ingredient).
