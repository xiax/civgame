...
- **Paleolithic hearth pacing:** `paleolithic_hearth_count` (`settlement.rs`) is now an upper bound (`(members+5)/6`, no cap), not a hard limit. The actual queue gate in `generate_candidates` opens a new hearth only when `find_bed_tile_around_hearth` returns `None` for every existing hearth (crescents saturated) AND `bed_deficit_pre > 0` (members still lack beds). Result: bands fill out hearth #1's bed crescent before hearth #2 is even queued. Settled factions still target a single civic-zone hearth — hearth-per-house requires a Household component and is future work.
