...
- **Teaching (passive):** `tech_teaching_system` (Economy, after gossip) scans pairs of socializing agents within 3 tiles, finds techs the teacher has Learned that the student is Aware of but not Learned, and rolls a low chance (`0.004 × INT_scale`). On success the highest-complexity teachable tech transfers to the student's Learned set (subject to capacity). Untouched by Phase-2; remains the ambient shortcut.
- **Phase-2 directed teaching (`teaching.rs`):** Three new pathways feed the same `study_progress` map. (1) **Read** — `TaskKind::Read`, `tech_focus = Some(tech)`. `read_task_system` adds `+1 × INT_scale` per tick. Player triggers via inspector "Read <tech>" button on Aware-only rows that have a matching tablet/book in inventory; `apply_player_knowledge_orders_system` consumes `PlayerOrderKind::ReadItem` and inserts `Drafted` so dispatch leaves them alone. (2) **Lecture** — inspector "Lecture" button on a Learned-tech row writes `LectureRequest(Some((lecturer, tech)))`. `apply_lecture_request_system` (Economy) drafts up to 8 same-faction adults within 6 tiles, drops their `ActivePlan` mirror-of-`apply_muster_hunters_system`-style, and inserts `Lecturing { ends_tick=now+600, tech, anchor }` + `Attending`. `lecture_tick_system` (Sequential, after movement) credits `+2 × INT_scale` per tick to each attending student and releases on threshold/timeout/distance. (3) **1-on-1 teach** — right-click a friendly person → "Teach". `PlayerOrderKind::Teach(student)` routes the teacher adjacent; `apply_teach_order_system` then inserts `TeachingPair`/`BeingTaught` with `ends_tick=now+120`. `teach_task_system` credits `+3 × INT_scale` per adjacent tick and emits `ActivityEntryKind::Taught` on success. All three pathways carry `Drafted` while busy.
- **Faction-tech derivation:** `FactionData.techs` is a *cache* now — `sync_faction_techs_from_chief_system` (Economy) projects the chief's `aware` bitset onto it every tick. The 39 existing read sites continue working unchanged: most ask "can the faction direct this?" → chief-aware suffices. Per-person execution gates were migrated:
  - Plan candidate filter (`plan/mod.rs:plan_execution_system`) checks the worker's `has_learned(tech_gate)` — a worker can only run a plan whose tech they've personally mastered.
  - `job_claim_system` (`jobs.rs:892`) checks the worker's `has_learned(recipe.tech_gate)` before claiming a `JobKind::Craft` posting — Phase 2 tightens craft gating per-worker, not just per-faction.
  - `faction_hunter_assignment_system` only promotes `Profession::None` candidates with Learned `HUNTING_SPEAR`.
  - `mount_check_system` requires per-person Learned `HORSEBACK_RIDING`.
- **Tablets and books (`Item.tech_payload: Option<TechId>`):** `Good::ClayTablet` (id 20, `OneHand`, 1500 g) and `Good::Book` (id 21, `Small`, 600 g). Crafted via recipes 10/11 in `CRAFT_RECIPES` (gated on `CUNEIFORM_WRITING`, Workbench station). `CraftOrder.tech_payload` and `JobProgress::Crafting.tech_payload` thread the chosen TechId from the chief-posting branch through to `craft_order_system`'s completion stamp where it lands on the produced `Item`. Tablets and books are reusable — reading does not consume them. Item equality partitions tablets-of-tech-A from tablets-of-tech-B in inventory and ground-item piles automatically (`spawn_or_merge_ground_item_full` merges by full `Item`). Recipes `RECIPE_CLAY_TABLET=10` and `RECIPE_BOOK=11` plus helper `recipe_encodes_knowledge(recipe_id)` exposed from `crafting.rs`.
- **Tablet posting (`chief_tablet_posting_system`, `jobs.rs`):** Runs every `CHIEF_TABLET_POSTING_INTERVAL=3600` ticks (one game-day) per faction. Reads chief's `learned`, tallies adult-awareness across all faction members, picks the highest-complexity bit the chief Learned that <50% of adults are Aware of (skipping techs already covered by a live tablet posting), and posts a `JobKind::Craft` with `recipe=10` and `tech_payload=Some(chosen)`. Player override: `PlayerCraftRequest` resource — written by inspector "Encode" button or `PlayerOrderKind::EncodeTablet`; consumed every tick (regardless of cadence) and posted with `JobSource::Player`/priority 180.
- **Tech tree (`technology.rs`):** 43 techs, 5 eras, prereq DAG, per-tech `triggers: &[TechTrigger]` mapping `ActivityKind → per_unit_chance`. `TechBonus` adds yield/storage/combat bonuses; bonuses still aggregate via `faction.techs` (chief-aware). Old season-boundary `tech_discovery_system` was deleted; activity_log on FactionData is still incremented for the debug panel readout but no longer drives tech.
- **UI:** Tech panel (`ui/tech_panel.rs`) shows tri-state per tech — ✓ green (chief Learned), ◉ teal (chief Aware only), ◎ yellow (discoverable), ○ gray (locked) — plus a hover row "Complexity: N pt(s) · Members: N/M learned, N aware". Inspector has a Knowledge section listing the selected person's Learned/Aware sets and capacity used/total. Each Learned row carries small "Lecture" / "Encode" buttons that fire `InspectorActionKind::HoldLecture` / `EncodeTablet`; each Aware-only row whose tech matches an inventory tablet/book carries a "Read" button (`ReadItem`). An "In progress" sub-section lists `study_progress` entries as `tech: progress/threshold ticks`. Right-click a friendly person → "Teach" routes via `PlayerOrderKind::Teach`. New `ActivityEntryKind::Taught { student_name, tech_name }` and `Read { tech_name }` events render in the activity log.

### Faction systems

- **Factions (`faction.rs`):** `FactionTechs` bitset is a derived projection of the chief's `PersonKnowledge.aware` (see Knowledge & technology); rebuilt every Economy tick by `sync_faction_techs_from_chief_system`. `FactionCenter`, bonding, storage rollup, raids. `SOLO=0` = ungrouped. `StorageTileMap` indexes storage tiles per faction. `FactionStorage::totals` refreshed every Economy tick.
