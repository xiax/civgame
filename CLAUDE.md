...
**Phase H beliefs**: `PersonKnowledge.belief: AHashMap<BeliefGroupId, BeliefState>`. Seeded by `seed_initial_beliefs(target_era)`: pre-Neolithic → Sky Dome + Spirit Illness + Eclipse Omens; Neolithic+ → Geocentric + Miasma + Weather Omens. Beliefs never land in `learned` — `seeded_realistic_through_era` skips them in the Learned-bitset loop. `FactionData.chief_disease_belief` / `chief_cosmology_belief` cache the chief's accepted ids; refreshed every Economy tick by `sync_faction_techs_from_chief_system`. **Consumer hooks** (`organic_settlement::pressure_to_intent` + `medicine::htn_seek_care_dispatch_system` + `mood::derive_mood_system`):
- `MIASMA_THEORY` accepted → +30% `WaterAccess` intent priority (Miasma-driven sanitation reflex builds wells earlier).
- `SPIRIT_ILLNESS` accepted → +30% `Ritual` intent priority (Shrines earlier) AND injured patient skips the home_tile fallback in `htn_seek_care_dispatch_system` (Spirit-Illness believers refuse non-ritual at-home treatment; Healer-side radius scan still reaches them).
- Personally-accepted `ECLIPSE_OMENS` + `Calendar::eclipse_active()` (sun-occluded daylight, even-year last-day-of-summer, day-fraction 0.35..0.55) applies `ECLIPSE_OMEN_MOOD_PENALTY = 25` mood hit; Weather-Omens / no-belief agents shrug. Eclipse cadence in `world/seasons.rs::is_eclipse_today` / `eclipse_active`.
...
**Diplomacy & Territory (`simulation::{territory, diplomacy, trespass}`):** Sparse per-tile `TerritoryMap` derived from live `Settlement` + pitched `Camp` anchors; era-keyed radius lifted by √peak_population, falloff produces Claimed/Contested cells. `DiplomacyLedger` (faction-pair) holds `TreatySet` (Trade/Alliance/NonAggression/War — War is exclusive) + four `Reputation` tracks (trust/fear/grievance/familiarity) with daily half-life decay. AI proposes/responds on `TICKS_PER_DAY`/`TICKS_PER_DAY/4` cadences via pure-fn `evaluate_proposal`. Trespass detection rides `Changed<Transform>` after `sync_indexed_after_move_system`; per-pair throttled. Warning → grievance → defender draft escalates via `TerritoryDefenseQueue`. `goal_update_system` flips Defend on queue non-empty; HTN `Defend` arm prefers the nearest queued tile over home. `raid::pick_raid_target` skips allied/trade/non-aggression factions; successful raids stamp `IncidentKind::Raid` and `declare_war` on the pair. Player commands `SendDiplomacyProposal` / `RespondDiplomacyProposal` / `DeclareWar` / `BreakTreaty` are faction-level (mirror `EncodeTablet` shape). HUD "Diplomacy" button toggles `diplomacy_panel`; world-map `show_territory` overlay tints by owner. See `src/simulation/CLAUDE.md` → Diplomacy & Territory.
