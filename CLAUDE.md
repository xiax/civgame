...
- **`Cropland`** — tilled farm soil. **Only ever appears inside an Agricultural plot.** **Worked into existence** by `farm::prepare_field_task_system` (Sequential, `FIELD_PREP_WORK_TICKS=80`). `carve_plots_system` + `seed_starting_farms_system` populate `PlotIndex.ag_tiles` and seed per-tile entries in `farm::FieldTileIndex` but leave the underlying soil/grass; founders pay Spring 1 to till. `seed_starting_farms_system` does pre-till a bounded contiguous starter patch inside the belt plot (Cropland kind only — natural world-gen fertility, never overridden). There is no seed-time house yard — personal kitchen gardens are real Agricultural parcels (see `organic_settlement::append_dwelling_gardens`). `is_soil_like`, speed 0.9, and **never paved by road carving**.
