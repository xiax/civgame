...
Catalog of 91 `TechId`/`KnowledgeId` entries spanning four axes:
...
**Phase BN — biome-native plant ecology** (86–90): `PLANT_LORE` (Mesolithic Subsistence skill) gates papyrus → paper; `HERBAL_MEDICINE` (Neolithic Medicine skill) gates `medicine_bundle` (healing accelerant, halves Injury severity time) + dye → luxury; `FIBER_PROCESSING` (Neolithic Craft technique) gates fibre → cloth alongside the legacy grain → cloth recipe; `ORCHARD_CULTIVATION` (Neolithic Subsistence technique; prereq `CROP_CULTIVATION`) gates latex → rubber; `OIL_PRESSING` (Chalcolithic Craft technique; prereq `FIRED_POTTERY`) gates oilseed → plant_oil.

## Biome-native plants (`simulation/plant_catalog.rs`, `world/flora_regions.rs`)

Data-driven species catalog at `assets/data/plants/core.ron`; ~50 species including the three legacy keys (`emmer_wheat`, `generic_berry_bush`, `oak_tree`) which pin `PlantKind::{Grain, BerryBush, Tree}` defaults.

- **`PlantSpeciesId(u16)`** + `PlantCatalog` resource (alphabetical sort → deterministic ids). `Plant.kind: PlantKind` remains as a coarse legacy form bucket (annual food crop / perennial shrub / tree); the sibling `PlantSpecies(PlantSpeciesId)` component carries the catalog id. `spawn_plant_at` defaults to `catalog.default_for_kind(kind)`; `spawn_plant_at_species` lets callers pick a specific species.
- **`PlantDef`** carries `form: PlantForm`, `uses: Vec<PlantUse>`, `native_realms: Vec<FloraRealmKind>`, `spawn: PlantSpawnRule { biomes, surface_tiles, fertility, river_distance, requires_coastal, base_weight }`, `lifecycle: PlantLifecycleProfile { annual, sowing_seasons, stage_thresholds, season_growth, … }`, `harvests: Vec<PlantHarvestProfile { trigger, tool, yields, despawn, stage_after, … }>`, `seed: Option<ResourceId>`, `farm_tile_class`.
- **Multi-profile harvest:** trees that fruit (oak/crabapple/cacao/mesquite/coconut/pinyon/date) carry both `OnFruitSeason(Autumn) → bare-hands → nuts/fruit, plant remains` AND `OnFell → Axe → wood, despawn`. `PlantCatalog::pick_harvest_profile(species, stage, season, has_tool, prefer_despawn)` resolves the right one. `gather_system` consults this via `resolve_species_harvest_profile` — `prefer_despawn` reads the agent's `active_gather_claim.kind` (`MemoryKind::wood()` ⇒ true), so an axe-carrying wood hauler doesn't shake an oak for acorns. Yields/despawn/stage_after/skill/activity all flow from the resolved profile; legacy 3-kind species (emmer/berry/oak) keep the `PlantKind::harvest_*` path so grain's nutrient debit + Tree's no-Axe deadwood fallback stay intact.
- **Per-species lifecycle:** `plant_lifecycle_system` reads `PlantLifecycleProfile` when `PlantSpecies` present — `season_growth` / `stage_thresholds` / `wild_sprout_chance` / `scatter_chance` / `scatter_radius` / `regrow_after_harvest` / `annual` all per-species. Winter mortality keys on `profile.annual` (legacy: only Grain). `try_scatter_seed` now spawns the same species at offspring tiles (Crabapple → Crabapple seeds). `seed_target_tile_ok` runs the full per-species spawn-rule cascade — `surface_tiles`, `river_distance` (riparian band), `requires_coastal` (ocean adjacency), and the precomputed `native_pools[(realm, biome)]` membership — when a species is supplied and the system can resolve `Res<Globe>`. Cultivated and wild scatter share the same gate so imported crops can't naturalise outside their native floristic region. Headless test fixtures without a `Globe` fall back to surface+river-only checks.
- **Memory tagging:** `plants::plant_memory_kinds(species, fallback_kind)` walks every harvest profile and returns each distinct `MemoryKind` the plant should be sighted under — foods canonicalise to `AnyEdible`; every non-food primary yield gets `MemoryKind::Resource(rid)`. `vision_system` writes every channel in one sweep so multi-profile species (oak fruit + wood) seed both clusters and independent HTN gather methods (`AcquireGood{fiber|medicinal_herb|resin|…}`) can find them.
- **Floristic regions (`world/flora_regions.rs`)**: `Globe.flora_regions: FloraRegionMap` built once in `generate_globe` (after `build_relief`) via flood-fill of land cells partitioned by latitude band + dominant biome. Each `FloraRegion { realm: FloraRealmKindWire, flavor_name, … }`; `floristic_region_at_tile` is the per-tile accessor. `GLOBE_FILE_VERSION = 11`. Deterministic across runs of the same seed — the build walks AHashMap-grouped cells through a `(lat, biome)`-sorted promotion pass so per-process hasher state can't leak into region ids. `Biome` derives `Hash`.
- **Native pool cache** (`PlantCatalog.native_pools`): `from_defs` precomputes per-`(FloraRealmKind, Biome)` species lists; `native_pool_for(realm, biome)` is the O(1) accessor. `chunk_streaming::spawn_chunk_plants` walks this slice (typically 0-12 species) instead of all ~50 species per tile.
- **Chunk spawn lottery (`chunk_streaming::spawn_chunk_plants`)**: per tile, reads `catalog.native_pool_for(realm, biome)` then applies per-tile gates (surface_tiles, fertility, coastal, **river_distance via `chunk_map.river_distance_at`**, patch_noise, base_weight). Weighted lottery plus a per-biome `EMPTY_BIAS` so undecided rolls leave the tile empty.
- **Founder native seeds** (`seed_starting_farms_system`): in addition to the legacy emmer grain bundle, founders born into a region get a 4-unit seed bundle per native plantable species (up to 4 species). `floristic_region_at_tile(home)` → walks the catalog for `def.seed.is_some() && def.native_realms.contains(realm)`. Excludes `grain_seed` to avoid double-stocking.
- **Farm seed picker** (`htn_plant_from_storage_dispatch_system`): walks every catalog species with a `seed` resource, filters by `is_sowable_in(cur_season)`, picks the highest in-stock seed. Legacy `PlantKind::ALL` walk kept as a backstop for fixtures that bypass the catalog.
- **Wire (`PROTOCOL_VERSION = 8`)**: `TileOverlayOp::AddPlant` carries `species: u16` alongside the legacy `kind: PlantKindWire` so clients without a loaded catalog can fall back to coarse rendering.
- **Per-species sprites** (`rendering/plant_sprites.rs`, Phase 6 of biome-native plants): `PlantSpriteSet` resource probes `assets/textures/plants/<folder>/<stage>_<variant>.png` at startup via `Path::exists()` — Bevy's `AssetServer::load` is bypassed for missing files so the resolver can route to a higher fallback tier. `PlantDef.sprite_keys: PlantSpriteKeys { folder, variants }` (RON, default empty) overrides the species `key` as the folder slug and the default 3-variant layout. `PlantSpriteVariant(u8)` component stamped at spawn from `splitmix64(species, tile) % 3` — persistent across stage transitions so a Mature oak that started Seedling variant 1 stays variant 1. `entity_sprites::resolve_plant_sprite` three-tier fallback: species PNG → form PNG (`_form_<form>/`) → legacy ASCII (`get_plant_texture_legacy`). Stage layout: Seed/Harvested single-variant; Seedling/Mature/Overripe 3-variant. Per-form canvas sizes (form-fallback PNGs ship at these dimensions; per-species PNGs follow the same per-form size, except trees which are tiered): Grass/Forb/Vine/Tuber 16×16, Shrub 24×24, Cactus 16×32, Aquatic 16×24, Tree 48×80 (Medium baseline; per-species art may upgrade to Small 32×48, Large 64×112, or Palm 32×96). `Anchor::BottomCenter` on every plant sprite so trunks plant at the tile and canopies overhang. Form-fallback art is procedurally generated by the `gen_form_fallback_pngs` ignored test in `rendering/plant_sprites.rs` — a small `Canvas` helper with `fill_rect`/`fill_ellipse` drives the templates; `variant_transform` derives v1 (mirror) and v2 (palette shift one band lighter) from v0. Authoring is incremental — `spawn_plant_sprites`/`update_plant_sprites` queries take `Option<&PlantSpecies>` / `Option<&PlantSpriteVariant>` so unauthored species and legacy raw-`Plant` test fixtures all route through the same resolver and render whatever tier has art available.
- **Plant-derived resources** (`assets/data/resources/plants.ron`): 32 entries including ~25 plant foods (acorn, banana, dates, coconut, taro, yam, etc.), generic `fiber`, `medicinal_herb`, `medicine_bundle`, `dye_plant`, `resin`, `oilseed`, `plant_oil`, `latex`, `rubber`, `birch_bark`, `papyrus_stalk`, `paper`, and 10 new seeds. Recipes 47–52 hook them into cloth/medicine/luxury/oil/paper/rubber pipelines (see Knowledge → Phase BN).
- **`medicine_bundle` accelerator**: `heal_task_system` consumes one bundle from healer inventory/hands at heal time to double `HEAL_LIMB_HP_PER_TICK`. Pure-activity healing still works without it.
...
Helpers: `stone_yield_count`, `soil_fertility_mult`. No `Farmland` variant — Grain grows on `Cropland`; world-gen `TileData.fertility` is immutable per-tile recovery ceiling; `FieldTileIndex.by_tile[tile].nutrients` is the live nutrient pool. `FieldTileIndex` carries a `by_plot` reverse index (access via `plot_tiles(pid)` / `plot_has_members(pid)`) so per-plot scanners avoid walking holes in the hull. The shared `farm::FarmWorkIndex` snapshot owns per-plot `{unprepared, plantable, mature}` counts — `goal_update_system`, `chief_job_posting_system` Farm branch, and `fieldwork_expiry_system` all read it instead of rescanning rects. Pathing speeds: Sand 0.75, Snow 0.6, Marsh 0.4, Scrub 0.9, soils 0.85–0.9 (Cropland 0.9), stone 1.0.
...
**Thin housing edge walls** (`world/edge.rs`, `plans/thin-edge-walls.md`) — permanent dwellings (huts/longhouses) put walls/doors on the *boundary edge between two tiles* instead of consuming whole tiles, so the full footprint stays passable floor. `EdgeKey{axis,x,y}` (canonical lower/left owner tile) + `EdgeState{Open/Wall/ClosedDoor/OpenDoor}`. Cheap per-`Chunk` cache `edge_bits: Option<Box<ChunkEdgeBits>>` (N/E edge per tile, 2 bits each) is read by `ChunkMap::{edge_blocks_move,edge_blocks_los_opaque}` *inside* `passable_step_3d`/`passable_step_for` (cardinal; diagonals gated by the corner legs) and LOS `walk_ray` transition test — zero new params ripple out. Durable truth = `EdgeStructureMap` (`EdgeWall{material,owner_faction}`/`EdgeDoorRef{open,faction,dir}`), re-stamped on chunk load by `restamp_edge_structures_on_chunk_load` (mirrors `WallMap`↔`TileKind::Wall`). Construction: `BuildSiteKind::{EdgeWall(WallMaterial),EdgeDoor}` reuse the `Wall`/`Door` recipes; `walled_house_tile_plan` emits **one `EdgeWall` blueprint per perimeter tile** carrying an `edge_sides: u8` bitmask (`edge_side::{N,E,S,W}`; corners=2 bits) so blueprints stay 1:1 with tiles (each boundary edge has one interior owner) and `BlueprintMap`/haul/claim are untouched; finalize stamps `EdgeStructureMap`+cache+`WallConstructed`. Reachability sim is edge-aware (`simulate_house_reachable_edges`, `ReachOpts.blocked_edge`). Full-tile `Wall`/`Door` (natural rock, palisades, manual, composite L/U houses) unchanged. **Floor keep-out (Phase 4):** `DwellingEnvelopeMap` (tile→`DwellingId` + per-dwelling `DwellingInfo{tiles,bbox}`) is registered with the full footprint rect at `plan_building` (covers immediate + deferred terraform paths) and `seed_walled_house_at`. Consulted by `organic_settlement::single_tile_clear` (so house/structure placement routes around occupied hut floors — the actual placement gate; `rect_clear_for_parcel` stays unaware since the parcel is only a zoning hint), `road_carve_system`'s `try_write_road` + connector planner (no road through a living room), and `well_footprint_clear` (runtime via `WellPlacementCtx.dwelling_envelopes`). `populate_seed_reservation_system` folds the envelope into `SeedReservation`, which routes seed-time plant/obstacle/stranded-member keep-out for free. Kitchen gardens: `SurveyStructureSnapshot.dwelling_rects` carries envelope footprints to the async survey so `append_dwelling_gardens` finds thin-wall huts (which have no full-tile walls for the legacy `detect_dwellings`). **Rendering (Phase 5):** `EdgeWallVisual`/`EdgeDoorVisual` durable entities spawn oriented thin-strip sprites at the edge midpoint via `entity_sprites::spawn_edge_{wall,door}_sprites` (vertical edge → N-S bar, horizontal → E-W bar; open door dimmed), projection y-sort via `auto_attach_dynamic`, art-mode-toggle aware. **Net replication (Phase 6, `PROTOCOL_VERSION=10`):** `TileOverlayOp::{AddEdgeWall,RemoveEdgeWall,AddEdgeDoor,RemoveEdgeDoor,SetEdgeDoorOpen}` (keyed by `EdgeKey`) ride the per-tick delta (`push_ops_for_tile` emits a tile's canonical N+E edges); bootstrap snapshot carries `edge_walls`/`edge_doors` for late joiners; client rebuilds `EdgeStructureMap` + stamps the edge cache + spawns visual stubs. `EdgeWallVisual`/`EdgeDoorVisual` auto-tag for `NetId`. **Deferred:** edge-wall destructibility/siege; tilted-view upright edge variant + edge-precise blueprint ghosts (stylistic); runtime edge-door open/close.
...
**Protocol (`PROTOCOL_VERSION = 8`, `net/protocol.rs`).** All serde-derived, ride `OrderedReliableChannel`:
