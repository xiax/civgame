...
- `era: Era` — every spawned member starts with all techs through this era Aware. The *Learned* set is scoped by founder role via `PersonKnowledge::seeded_realistic_through_era`: the first spawned member becomes the chief (Personal+Household+Subsistence+Specialist+Institutional Learned), every ~8th becomes a Specialist (adds Specialist+MilitaryTransport), the rest are Common (Personal+Household+Subsistence only). The era-blanket `seeded_through_era` is retained for tests/fixtures that need an "everyone knows everything" baseline. Structures and walls scale up accordingly.
...
End-to-end pipeline from chief-funded postings → escrow → worker payout → faction wage signal → EV-driven profession choice. See `plans/wage-aware-labor-market-v2.md` for phase-by-phase rationale and `src/simulation/CLAUDE.md` for subsystem detail.

- **Chief postings pay from faction treasury (Mixed/Market only).** `chief_post_funding_system` (Economy, after `chief_job_posting_system`) scans newly-posted chief contracts with `reward == 0.0`. For factions with a non-empty `economic_policy` map (Mixed/Market), it computes `chief_wage_for(progress)` (Stockpile/Haul/Craft → `trade_base_value(rid) × qty × CHIEF_MARGIN (0.5)`, Haul ×0.5 transport discount; Calories/Build/Planting → flat per-day formulas bounded by `CHIEF_BUILD_WAGE_CAP = 30`), debits `faction.treasury`, and spawns a `JobEscrow { amount, beneficiary: chief_entity }`. Unaffordable wages leave the posting at `reward = 0`. Subsistence factions (empty `economic_policy`) skip funding entirely so communal labor is preserved.
- **Worker payout closes the loop.** `job_payout_system` (Economy, exclusive `&mut World`, after `job_claim_release_system`) drains `Events<JobCompletedEvent { claimants, completed, target_rid }>`, looks up the escrow via `JobEscrowIndex(AHashMap<JobId, Entity>)`, and on `completed = true` splits `escrow.amount` evenly across claimants by direct credit (no `pay()` re-debit — funds are already held). Failed completions despawn the escrow with amount intact; the `JobEscrow` `on_remove` hook refunds `beneficiary`. Workers receive an `Earnings(VecDeque<EarningEntry { job_kind, target_rid, amount, tick }>)` ring (cap 16) plus an `ActivityEntryKind::WagePaid` log entry. Currency invariant `EconomicAgent + FactionData.treasury + Settlement.treasury + JobEscrow.amount` is preserved end-to-end.
- **Skills decay slowly toward peak-derived floors.** `Skills` is clamped at `SKILL_MAX = 255`; companion components `SkillPeaks` + `SkillUseTicks` ratchet peaks and stamp last-use ticks via `skill_peaks_tracker_system` (observer on `Changed<Skills>`). `skill_decay_system` runs daily with `SKILL_DECAY_HALF_LIFE_DAYS = 90`: skills above `SKILL_MASTERY_LINE (80)` decay toward `SKILL_MASTERED_FLOOR (30)`; below mastery they decay toward `max(SKILL_FLOOR_BASE (5), peak × 0.30)`. A mastered skill never falls below 30 — the agent's peak history is durable.
- **Capital recognition lifts EV for well-equipped agents.** `capital_factor(agent, carrier, tile, fid, household, prof, &ownership, &plots, &plot_index)` averages three orthogonal axes in `[1.0, 2.0]`: `tool_capital_factor` (`+0.5` when an inventory/hand item maps to `prof` via `tool_profession` — `weapon → Hunter`, `tools → Crafter`), `workshop_capital_factor` (`+1.0` for a household-owned profession-affine workshop within `WORKSHOP_AFFINITY_RADIUS = 12`, else `+0.5` for a village-owned one — `WorkshopKind::affine_to(prof)` keys Market→Bureaucrat, Workbench|Loom→Crafter), and `land_capital_factor` (Farmer only: `+0.5` when the household holds an Agricultural plot under non-StateOwned tenure). `OwnedBy { faction_id, kind, tile }` stamped at workshop finalize maintains a `WorkshopOwnership` resource via add/remove hooks.
- **Wage signal aggregates payouts per faction.** Each `FactionData` carries `wage_signal: AHashMap<(JobKind, Option<ResourceId>), WageEMA { ema_per_day, last_update_tick, samples }>`. `faction_wage_signal_system` (Economy, daily) walks each member's `Earnings` ring, sums last-day payouts per `(village, kind, rid)` triple, and folds into the EMA with `α ≈ 0.129` (5-day half-life). Pre-existing keys with no fresh sample decay toward zero — an outage shows up in the signal. Cross-faction perception lives on `PerceivedFactionWages` (cap 32, oldest-first), merged via `wage_gossip_system` piggybacking `Socialize` within `WAGE_GOSSIP_RADIUS = 3` with an `exp(-age / TICKS_PER_DAY)` staleness penalty.
- **Profession choice is EV-ranked.** `expected_wage(faction, prof, skills, capital_factor) = aggregate_wage_per_day × skill_competence(primary_skill) × capital_factor` where `skill_competence(s) = 0.2 + (s/SKILL_MAX) × 0.8`. The three target-driven systems — `faction_profession_system` (Farmer), `faction_hunter_assignment_system` (Hunter, gated on `HUNTING_SPEAR` + density), `chief_bureaucrat_appointment_system` (Bureaucrat, gated on `state_funds_public_works`), and `chief_craft_assignment_system` (Crafter, gated on `wage_signal[(Craft, _)].ema > 0`) — sort candidates by `(expected_wage, primary_skill)`. Wage signal dominates when populated; raw skill is the tiebreaker. Promotion picks the highest-EV `Profession::None`; demotion picks the lowest-EV current incumbent. Shared `demote_profession_state` releases reservations, cancels active tasks, and strips `Carrying`. `CRAFTER_AFFINITY_BONUS = 3.0` lifts a Crafter's paid-Craft `U_bid` above an equidistant generalist; unpaid path's `profession_bias` gets matching `(Crafter, Craft) = 0.5` arm.
- **Subsistence factions keep communal labor intact.** Empty `economic_policy` map → chief postings stay at `reward = 0`, household income skim is 0%, no wage signal accumulates, `chief_craft_assignment_system` short-circuits on the zero-signal gate. The wage-aware machinery is dormant — survival reflex and chief allocation work exactly as before. Mixed factions add private-actor postings on non-staples; Market factions go all-private with chief intervention only on Build/Defense.
