...
- **`building_template.rs`** ships `FootprintShape::{Rect, LShape, UShape}` + `Rotation` helpers; current `BuildIntent::Hut`/`Longhouse` use the rectangular path. `BuildIntent::Composite` is a future iteration.
